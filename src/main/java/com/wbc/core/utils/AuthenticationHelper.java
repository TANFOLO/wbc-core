...
public class AuthenticationHelper {
	private static Logger logger = LogManager.getLogger(AuthenticationHelper.class);  
	/**
	 * Logger de la classe
	 */
	...
	public String PerformSRPAuthentication(String username, String password) {
		...
		try {
			...
			if (ChallengeNameType.PASSWORD_VERIFIER.toString().equals(initiateAuthResult.getChallengeName())) {
				...
				//LOGGER.info(result);
				logger.info("PerformSRPAuthentication=" + result.getAuthenticationResult().getIdToken());
	...
	private InitiateAuthRequest initiateUserSrpAuthRequest(String username) {
		...
		initiateAuthRequest.addClientMetadataEntry("aa", "bb");
		initiateAuthRequest.addAuthParametersEntry("cc", "dd");
	...
	/**
	 * Récupère l'UUID du user à partir de son tocken
	 * @param accessToken
	 * @param url_json
	 * @return
	 */
	public static String getUUID(String accessToken , String url_json) {

		try {
			ConfigurableJWTProcessor jwtProcessor = new DefaultJWTProcessor();
			RemoteJWKSetTimeout keySource = new RemoteJWKSetTimeout(new URL(url_json));
			...
			JSONObject json = claimsSet.toJSONObject();

			String user_id = json.getAsString("sub");
			logger.info("user_id = " + user_id);
			return user_id;
		} catch (com.nimbusds.jwt.proc.BadJWTException e) {
			logger.error("BadJWTException {}", e);
		} catch (MalformedURLException e) {
			logger.error("MalformedURLException {}", e);
		} catch (ParseException e) {
			logger.error("ParseException {}", e);
		} catch (BadJOSEException e) {
			logger.error("BadJOSEException {}", e);
	...
	public static boolean checkJWTToken(String accessToken , String url_json) {

		ConfigurableJWTProcessor jwtProcessor = new DefaultJWTProcessor();

		JWKSource keySource = null;
		try {
			keySource = new RemoteJWKSetTimeout(new URL(url_json));
			...
			Iterator<Entry<String, Object>> claims = claimsSet.getClaims().entrySet().iterator();
			while (claims.hasNext()) {
				Entry<String, Object> claim = claims.next();
				logger.info("claim = {} {}", claim.getKey(), claim.getValue());
			}

			JSONObject json = claimsSet.toJSONObject();

			String user_id = json.getAsString("sub");
			String email_verified = json.getAsString("email_verified");
			String newsletter = json.getAsString("custom:newsletter");
			String username = json.getAsString("cognito:username");
			String locale = json.getAsString("locale");
			String aud = json.getAsString("aud");
			String event_id = json.getAsString("event_id");
			String updated_at = json.getAsString("updated_at");
			String token_use = json.getAsString("token_use");
			String auth_time = json.getAsString("auth_time");
			String exp = json.getAsString("exp");
			String iat = json.getAsString("iat");
			...
			//LOGGER.info("exp = " + dt.format(new Date(exp)));

			logger.info("exp = " + dt.format(claimsSet.getExpirationTime()));
			return true;
